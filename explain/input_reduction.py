...
@Attacker.register("custom-reduction", exist_ok=True)
class InputReduction(Attacker):
    ...
    def attack_from_json(
        ...
        original_text_field: TransformerTextfield = original_instances[0][  # type: ignore
            input_field_to_attack
        ]
        original_tokens = deepcopy(original_text_field.input_ids)
        final_tokens_and_indices = []
        for instance in original_instances:
            final_tokens_and_indices.append(
                self._attack_instance(
                    inputs, instance, input_field_to_attack, grad_input_field, ignore_tokens
                )
            )

        final_tokens = [x[0] for x in final_tokens_and_indices]
        final_indices = [x[1] for x in final_tokens_and_indices]
        final_logits = [x[2] for x in final_tokens_and_indices]
    ...
    def _attack_instance(
        ...
        text_field: TransformerTextfield = instance[input_field_to_attack]  # type: ignore
        current_tokens = deepcopy(text_field.input_ids)
        current_indices = deepcopy(instance["metadata"]["indices"])
        current_logits = None
        ...
        while len(current_tokens) > num_ignore_tokens and candidates:
            ...
            def get_length(input_instance: Instance):
                input_text_field: TransformerTextfield = input_instance[input_field_to_attack]  # type: ignore
                return len(input_text_field.input_ids)
            ...
            for beam_instance, smallest_idx, tag_mask in beam_candidates:
                ...
                if "tags" not in instance:
                    ...
                    red_probs = np.exp(outputs['logits'][0])
                    red_probs /= np.sum(red_probs)

                    if utils.instance_has_changed(beam_instance, fields_to_compare) or red_probs < .5:
                        continue
                ...
                current_tokens = deepcopy(text_field.input_ids)
                current_indices = deepcopy(beam_instance["metadata"]["indices"])
        ...
        return current_tokens, current_indices, current_logits
...
def _remove_one_token(
    ...
    for token_idx, token in enumerate(text_field.input_ids):
        if int(token) in ignore_tokens:
    ...
    for _ in range(beam_size):
        ...
        def delete_smallest(field):
            value_before_smallest = field[0:smallest]
            value_after_smallest = field[smallest + 1:]
            return torch.cat((value_before_smallest, value_after_smallest))

        copied_text_field.input_ids = delete_smallest(copied_text_field.input_ids)
        copied_text_field.token_type_ids = delete_smallest(copied_text_field.token_type_ids)
        copied_text_field.attention_mask = delete_smallest(copied_text_field.attention_mask)

        indices_before_smallest = copied_instance["metadata"]["indices"][0:smallest]
        indices_after_smallest = copied_instance["metadata"]["indices"][smallest + 1:]
        new_indices = indices_before_smallest + indices_after_smallest

        metafield = MetadataField({"model_tokens": copied_instance["metadata"]["model_tokens"],
                                   "word_ids": copied_instance["metadata"]["word_ids"],
                                   "word_pieces": copied_instance["metadata"]["word_pieces"],
                                   "wordpiece_relation_indices": copied_instance["metadata"][
                                       "wordpiece_relation_indices"],
                                   "transformed_relation_indices": copied_instance["metadata"][
                                       "transformed_relation_indices"],
                                   "indices": new_indices})

        copied_instance.add_field("metadata", metafield)
